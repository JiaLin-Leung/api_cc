...
class Hub:
    def __init__(self, request=None, headers=None, cookies=None):
        ...
        self.cookies = cookies or {}
        if request:
            token = settings.SESSION_COOKIE_NAME or request.META.get('HTTP_TBKT_TOKEN') or request.COOKIES.get('tbkt_token')
    ...
    def __getattr__(self, alias):
        ...
        self.headers['req_referer'] = settings.DOMAIN
        # if not settings.DEBUG:
        #     self.headers['Host'] = url_root[url_root.find("//")+2:]
        #     self.headers['Connection'] = "close"
        #     url_root = settings.TBKT_HOST
        TBKT_HOST = settings.TBKT_HOST
        if TBKT_HOST and TBKT_HOST != "":
            url_root = settings.TBKT_HOST

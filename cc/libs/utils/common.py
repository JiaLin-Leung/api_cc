...
def replace_str(obj_str, r_type):
    """
    功能:替换数据中的特殊字符
    :param obj_str: 待替换的字符串
    :param r_type: 替换类型:1-替换数据中的单引号为"@#&!",2-替换数据中的"@#&!"为单引号
    :return:
    """
    if r_type == 1:
        if re.findall("'", obj_str):
...
def get_portrait(portrait, user_type):
    ...
    if portrait:
        # 获取文件的绝对路径
        return get_absurl(portrait, 3)
    else:
        if user_type == 1:
            # 学生
            return '%s/stu_default.png' % settings.FILE_CDN_URLROOT
        elif user_type == 2:
            # 家长
            return '%s/stu_default.png' % settings.FILE_CDN_URLROOT
        elif user_type == 3:
            # 教师
            return '%s/tea_default.png' % settings.FILE_CDN_URLROOT
...
def page_db(data_list, page_no, page_size=10):
    """
    分页 针对使用db从查询的结果分页
    :param data_list: 需要分页的数据结果
    :param page_no: 页码
    ...
    """
    data = {
        'page': data_list[page_size * (page_no - 1):page_no * page_size],
        'count': len(data_list),
        'sum_page': math.ceil(len(data_list) / page_size)
    }
    return data
...
def get_serializer_first_errors_msg(instance):
    """
    用于返回序列化器校验信息
    if not serializer.is_valid():
        return ajax.ajax_fail(message=get_serializer_first_errors_msg(serializer.errors))
    """
    return str(list(instance.values())[0][0])


def judge_real_name_contain_len(real_name, min_len, max_len):
    """
    功能:全部是中文
    ...
    try:
        ...
        name_list = []
        if min_len <= user_name_len <= max_len:
